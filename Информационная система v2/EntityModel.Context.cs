//------------------------------------------------------------------------------
// <auto-generated>
//     Этот код создан по шаблону.
//
//     Изменения, вносимые в этот файл вручную, могут привести к непредвиденной работе приложения.
//     Изменения, вносимые в этот файл вручную, будут перезаписаны при повторном создании кода.
// </auto-generated>
//------------------------------------------------------------------------------

namespace Информационная_система_v2
{
    using System;
    using System.Data.Entity;
    using System.Data.Entity.Infrastructure;
    
    public partial class EntityModelContainer : DbContext
    {
        public EntityModelContainer()
            : base("name=EntityModelContainer")
        {
        }
    
        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            throw new UnintentionalCodeFirstException();
        }
    
        public virtual DbSet<Device> DeviceSet { get; set; }
        public virtual DbSet<DoctorMessage> DoctorMessageSet { get; set; }
        public virtual DbSet<Doctor> DoctorSet { get; set; }
        public virtual DbSet<Patient> PatientSet { get; set; }
        public virtual DbSet<Administrator> AdministratorSet { get; set; }
        public virtual DbSet<MeasuredData> MeasuredDataSet { get; set; }
        public virtual DbSet<HealthStatus> HealthStatusSet { get; set; }
        public virtual DbSet<DeviceMessage> DeviceMessageSet { get; set; }
    }
}
